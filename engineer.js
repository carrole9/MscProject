...
function addDropDownFeatures4(){	
    ...
    $.ajax({
			...
			//alert(data);
			             for(i=0;i<data.length;i++){
						 var x = document.getElementById("myIMSI1");
						// var y = document.getElementById("FmyIMSI");
					     var option = document.createElement("option");
						    option.text = data[i];
						    x.add(option);
...
function addDropDownFeatures5(){	
    ...
    $.ajax({
			...
			//alert(data);
			             for(i=0;i<data.length;i++){
						 var x = document.getElementById("FmyIMSI1");
						// var y = document.getElementById("FmyIMSI");
					     var option = document.createElement("option");
						    option.text = data[i];
						    x.add(option);
...
//		document.getElementById("myIMSIDisplay").style.visibility = "hidden";
	     ...
	     $.ajax({
	    	async:false, 
				...
				success: function(data){
					...
					$(".odd").remove();
					$(".even").remove();
					document.getElementById("myEventID1").style.display = "table";
					$.each(data, function(i, basedatas){
						...
						$.each(basedatas, function(i,basedata){
			                ...
			                $("#myEventID1 tbody").append(row);
						})
				})
					
				$("#myEventID1").trigger('update');
	...
	//user story 5 CS
	$('#submitFailureCount1').on('click', function(){	

		var my_arr = [];
		my_arr.push($("#x").val());
		my_arr.push($("#y").val());
		my_arr.push($("#FmyIMSI1").val());
	    var jsonString = JSON.stringify(my_arr);
	    // alert(jsonString);
...
//					document.getElementById("myCauseCode").style.visibility = "visible";
	...
	$("#datetimepicker18").on("dp.change", function (e) {
	    $('#datetimepicker19').data("DateTimePicker").minDate(e.date);
	});
	$("#datetimepicker19").on("dp.change", function (e) {
	    $('#datetimepicker18').data("DateTimePicker").maxDate(e.date);
	});
...
//		document.getElementById("myEventID").style.visibility = "hidden";
		$(".odd").remove();
		$(".even").remove();
		document.getElementById("myEventID1").style.display = "none";
		$("#myEventID1").trigger('update');
	});
